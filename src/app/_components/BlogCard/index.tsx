    ...
    `}>
      ...
      <div className="p-6 flex flex-col gap-4">
        <header className="flex flex-col gap-4">
          <Link href={`/blog/${slug}`}>
            <h2 className="font-bold text-xl hover:text-slate-900 transition-colors">{title}</h2>
          </Link>
        ...
        <div className="flex flex-col gap-2">
          <Link href={`/blog/${slug}`}>
            <p className="font-semibold text-md text-teal-500 hover:text-teal-600 transition-colors">Read More</p>
          </Link>
          {media.attribution && (
            <p><small>Photo by {media.attribution}</small></p>
          )}      
        </div>
      </div>
    </article>
